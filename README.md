...
As required by the assignment guidelines, multiple regressors were systematically benchmarked against the rigorous temporal out-of-fold validation splits. The models were evaluated based on their ability to predict True Crop Production (Metric Tons). 

**The Optuna-Tuned XGBoost model emerged as the champion algorithm:**

| Regressor Model Architecture | Root Mean Squared Error (RMSE) | Mean Absolute Error (MAE) | R² Score |
| :--- | :---: | :---: | :---: |
| Baseline Ridge Regression | 3450.41 | 480.92 | 0.4510 |
| Random Forest Regressor | 2750.88 | 310.45 | 0.6120 |
| LightGBM Regressor | 2510.60 | 280.90 | 0.6402 |
| **🏆 XGBoost (Optuna Tuned)** | **2407.05** | **256.74** | **0.6882** |

*Note: The final evaluation metrics for the deployed champion model can be directly audited in the `data/artifacts/model_metadata.json` payload generated by the export pipeline.*
